...
export async function POST(request) {
  try {
    ...
    // can be duplicate-checked without a query per row.
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const dupByCvr = businessNumber ? byBusinessNumber.get(businessNumber) : null;
      const dupByName = !dupByCvr ? byName.get(normalizedName) : null;
      const duplicate = dupByCvr || dupByName;

      if (duplicate) {
        skipped.push({
          row: rowNum,
          storename,
          matchedField: dupByCvr ? "businessNumber" : "storename",
          existingStoreId: String(duplicate._id),
        });
        continue;
      }
      ...
      try {
        ...
        // Register it immediately so a later duplicate row in the same
        // file is caught too, not just duplicates against pre-existing data.
