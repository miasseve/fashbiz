...
export async function GET() {
  try {
    ...
    const users = await User.find({ role: { $ne: "admin" } })
      .select(
        "firstname lastname email role storename brandname isActive isVerified isProfileComplete phone country city address zipcode businessNumber products createdAt latitude longitude addedByAdmin"
      )
