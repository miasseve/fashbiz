...
export async function POST(req) {
  try {
    ...
    if (action === "create") {
      result = await subscriptionService.createSubscription(userId, priceId, paymentMethodId);
    ...
    if (action === "change") {
      ...
      return NextResponse.json({ success: true, message: "Plan updated", subscription: result });
    }
   
    if (action === "cancel") {
      result = await subscriptionService.cancelSubscription(userId);
      return NextResponse.json({ success: true, message: "Subscription will cancel at period end", subscription: result });
    }
